<#
.SYNOPSIS
    Windows System Repair Script (MECM/Remote Deployment Version) - Runs DISM and SFC to repair system issues
...
.DESCRIPTION
    ...
    This version is designed for non-interactive remote deployment through MECM/SCCM.
    All prompts are removed and the script runs automatically.
...
.PARAMETER LogPath
    Optional path for log files. Defaults to C:\Windows\Temp\SystemRepairLogs
...
.PARAMETER AutoReboot
    Automatically reboot the computer after successful completion (with 60 second warning)

.PARAMETER ForceReboot
    Force immediate reboot after successful completion (no warning)

.EXAMPLE
    .\WinSysRepair-MECM.ps1
    Runs both DISM and SFC with default logging, no reboot

.EXAMPLE
    .\WinSysRepair-MECM.ps1 -AutoReboot
    Runs both repairs and reboots after 60 seconds if successful

.EXAMPLE
    .\WinSysRepair-MECM.ps1 -LogPath "C:\Logs\SystemRepair"
    Runs both repairs with custom log location

.EXAMPLE
    .\WinSysRepair-MECM.ps1 -SkipDISM
    Runs only SFC scan

.NOTES
    Author: Created for system maintenance and MECM deployment
    Requires: Administrator privileges
    Version: 1.0 (MECM Edition)
    Date: 2025-11-28

    MECM Exit Codes:
    0 = Success
    1 = Validation error (missing admin, invalid params)
    2 = DISM failed
    3 = SFC failed
    4 = Both DISM and SFC failed
#>

[CmdletBinding()]
param(
    [Parameter(Mandatory=$false)]
    [string]$LogPath = "$env:SystemRoot\Temp\SystemRepairLogs",
...
function Show-Summary {
    ...
    # Handle automatic reboot (MECM-specific)
    if ($Script:DismSuccess -or $Script:SfcSuccess) {
        if ($ForceReboot) {
            Write-ColorOutput "`nForceReboot parameter specified - Restarting computer immediately..." -Color Yellow
            Start-Sleep -Seconds 5
            Restart-Computer -Force
        } elseif ($AutoReboot) {
            Write-ColorOutput "`nAutoReboot parameter specified - Restarting computer in 60 seconds..." -Color Yellow
            Write-ColorOutput "User can cancel the restart from the Windows notification." -Color Gray
            shutdown.exe /r /t 60 /c "System repair completed. Computer will restart in 60 seconds." /d p:0:0
        } else {
            Write-ColorOutput "`nRECOMMENDATION: Restart the computer to complete the repairs." -Color Yellow
            Write-ColorOutput "No reboot parameters specified - system will NOT restart automatically." -Color Gray
        }
    }
}
...
function Get-ExitCode {
    ...
    if (-not $SkipDISM -and -not $SkipSFC) {
        # Both were run
        if ($Script:DismSuccess -and $Script:SfcSuccess) {
            return 0  # Both succeeded
        } elseif (-not $Script:DismSuccess -and -not $Script:SfcSuccess) {
            return 4  # Both failed
        } elseif (-not $Script:DismSuccess) {
            return 2  # DISM failed
        } else {
            return 3  # SFC failed
        }
    } elseif (-not $SkipDISM) {
        # Only DISM was run
        return $(if ($Script:DismSuccess) { 0 } else { 2 })
    } elseif (-not $SkipSFC) {
        # Only SFC was run
        return $(if ($Script:SfcSuccess) { 0 } else { 3 })
    }
    return 0
}
...
Write-Host "Windows System Repair Script (MECM)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Administrator)) {
    ...
    Write-Host "MECM should run this with SYSTEM or admin account." -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
...
if ($ForceReboot -and $AutoReboot) {
    Write-ColorOutput "WARNING: Both ForceReboot and AutoReboot specified. Using ForceReboot (immediate restart)." -Color Yellow
}

# Initialize logging
Initialize-Logging

# Display what will be run (no confirmation prompt for MECM)
...
if ($ForceReboot) { Write-ColorOutput "  - Force Reboot (if successful)" -Color Yellow }
elseif ($AutoReboot) { Write-ColorOutput "  - Auto Reboot in 60s (if successful)" -Color Yellow }
Write-ColorOutput "`nProceeding automatically (MECM mode - no user prompts)..." -Color Green
Write-ColorOutput ""
...
if (-not $SkipDISM) {
    ...
    if (-not $dismResult) {
        Write-ColorOutput "`nWARNING: DISM encountered issues. Continuing with SFC..." -Color Yellow
    }
}
...
Write-ColorOutput "`nScript completed with exit code: $exitCode" -Color $(if ($exitCode -eq 0) { "Green" } else { "Yellow" })
